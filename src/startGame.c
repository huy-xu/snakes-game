...
snake* make_snake(int player_no, int head_y, int head_x) {
  ...
  game_map[head_y + 1][head_x] = game_map[head_y + 2][head_x] = player_no;
...
void move_snake(snake* s, direction d) {
  memmove(&(s->body_segment[1]), &(s->body_segment[0]),
...
void* gameplay(void* arg) {
  ...
  } while (!(((game_map[head_y][head_x] == game_map[head_y + 1][head_x]) ==
...
int main(int argc, char* argv[]) {
  ...
  // Set game borders
  for (i = 0; i < HEIGHT; i++) game_map[i][0] = game_map[i][WIDTH - 2] = BORDER;
  for (i = 0; i < WIDTH; i++) game_map[0][i] = game_map[HEIGHT - 1][i] = BORDER;

  // Randomly add five fruit
  srand(time(NULL));
  for (i = 0; i < 3; i++) add_fruit();

  // Create server socket
  socket_fds[0] = socket(AF_INET, SOCK_STREAM, 0);
  if (socket_fds[0] < 0) error("ERROR opening socket");

  if (setsockopt(socket_fds[0], SOL_SOCKET, SO_REUSEADDR, &(int){1},
                 sizeof(int)) < 0) {
  ...
  // order.
  socket_addr[0].sin_port = htons(atoi(argv[1]));
  ...
  for (i = 1;; i++) {
    ...
    if (socket_fds[i] < 0) error("ERROR on accept");
